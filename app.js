...
function updateFilterCount(){let n=0;if(ui.radius.value!=='2')n++;if(ui.age.value!=='90')n++;ui.filterCount.textContent=n;ui.filterCount.classList.toggle('show',n>0);}
function render(){updateFilterCount();if(!coords||!establishments)return;const radius=+ui.radius.value,maxAge=+ui.age.value,limit=50;let arr=establishments.map(e=>({...e,distance:hav(coords.lat,coords.lon,e.lat,e.lon),days:ageDays(e.date)})).filter(e=>e.distance<=radius&&e.days<=maxAge);arr.sort(sortMode==='closest'?(a,b)=>a.distance-b.distance||b.date-a.date:(a,b)=>b.date-a.date||a.distance-b.distance);const shown=arr.slice(0,limit);ui.summary.textContent=arr.length?`${arr.length.toLocaleString()} new · showing ${shown.length}`:'No new matches';if(!shown.length){ui.results.innerHTML='<div class="empty">No newly inspected establishments match these filters. Try a larger distance or older inspection window.</div>';return;}ui.results.innerHTML=shown.map((e,i)=>{const ds=e.distance<1?Math.round(e.distance*1000)+' m':e.distance.toFixed(1)+' km',map='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(e.name+' '+e.address+', Toronto'),cls=statusClass(e.result);return `<article class="item"><div class="rank">${i+1}</div><div class="topline"><div class="maincopy"><h3>${esc(e.name)}</h3><div class="address"><a href="${map}" target="_blank" rel="noopener" aria-label="Open ${esc(e.address)} in Google Maps (opens in a new tab)"><span class="address-text">${esc(e.address)}</span><span class="external-mark" aria-hidden="true">↗</span></a></div></div></div><div class="distance">${ds}<small>away</small></div><div class="inspection"><div class="datebox"><strong>${fmtDate(e.date)}</strong><span>${ageLabel(e.days)}</span></div><div class="inspection-copy"><div class="statusline ${cls}">${esc(e.result||'Inspection recorded')}</div><div class="type">First recorded DineSafe inspection</div></div></div></article>`;}).join('');}
