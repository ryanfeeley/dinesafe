...
async function loadData(){if(establishments)return establishments;status('<span class="spinner"></span>Loading new DineSafe establishments…');const r=await fetch(DATA_URL,{cache:'no-store'});if(!r.ok)throw new Error(`DineSafe data file returned ${r.status}`);const rows=parseCSV(await r.text());establishments=rows.map(row=>({id:get(row,['ESTABLISHMENT_ID']),name:get(row,['ESTABLISHMENT_NAME']),address:get(row,['ESTABLISHMENT_ADDRESS']),type:get(row,['ESTABLISHMENT_TYPE']),date:parseDate(get(row,['INSPECTION_DATE'])),result:get(row,['ESTABLISHMENT_STATUS']),inspectionCount:parseNum(get(row,['INSPECTION_COUNT'])),dining:get(row,['DINING'])!=='0',lat:parseNum(get(row,['LATITUDE'])),lon:parseNum(get(row,['LONGITUDE']))})).filter(e=>e.name&&e.date&&e.inspectionCount===1&&Number.isFinite(e.lat)&&Number.isFinite(e.lon));if(!establishments.length)throw new Error('The DineSafe file contains no establishments with exactly one recorded inspection.');const latest=establishments.reduce((m,e)=>!m||e.date>m?e.date:m,null);clearStatus();setDebug(`New establishments loaded: ${establishments.length}\nNewest first inspection: ${latest?.toISOString().slice(0,10)||'unknown'}`);return establishments;}
