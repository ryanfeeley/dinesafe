...
async function loadData(){if(establishments)return establishments;status('<span class="spinner"></span>Loading new DineSafe establishments…');const r=await fetch(DATA_URL,{cache:'no-store'});if(!r.ok)throw new Error(`DineSafe data file returned ${r.status}`);const rows=parseCSV(await r.text());establishments=rows.map(row=>({id:get(row,['ESTABLISHMENT_ID']),name:get(row,['ESTABLISHMENT_NAME']),address:get(row,['ESTABLISHMENT_ADDRESS']),type:get(row,['ESTABLISHMENT_TYPE']),date:parseDate(get(row,['INSPECTION_DATE'])),firstDate:parseDate(get(row,['FIRST_INSPECTION_DATE'])),result:get(row,['ESTABLISHMENT_STATUS']),inspectionCount:parseNum(get(row,['INSPECTION_COUNT'])),historicalCount:parseNum(get(row,['HISTORICAL_INSPECTION_COUNT'])),dining:get(row,['DINING'])!=='0',lat:parseNum(get(row,['LATITUDE'])),lon:parseNum(get(row,['LONGITUDE']))})).filter(e=>e.name&&e.date&&Number.isFinite(e.lat)&&Number.isFinite(e.lon)&&(e.inspectionCount===1||e.historicalCount===0));if(!establishments.length)throw new Error('The DineSafe file contains no establishments without historical records.');const latest=establishments.reduce((m,e)=>!m||e.date>m?e.date:m,null);clearStatus();setDebug(`New establishments loaded: ${establishments.length}\nNewest current inspection: ${latest?.toISOString().slice(0,10)||'unknown'}`);return establishments;}
function fmtDate(d){return new Intl.DateTimeFormat('en-CA',{month:'short',day:'numeric'}).format(d);}
...
function inspDetail(e){return e.inspectionCount>1?`First seen ${e.firstDate?fmtDate(e.firstDate):'in current feed'} · ${e.inspectionCount} inspections`:'First recorded DineSafe inspection';}
function updateFilterCount(){let n=0;if(ui.radius.value!=='2')n++;if(ui.age.value!=='365')n++;ui.filterCount.textContent=n;ui.filterCount.classList.toggle('show',n>0);}
function render(){updateFilterCount();if(!coords||!establishments)return;const radius=+ui.radius.value,maxAge=+ui.age.value,limit=150;let arr=establishments.map(e=>({...e,distance:hav(coords.lat,coords.lon,e.lat,e.lon),days:ageDays(e.date)})).filter(e=>e.distance<=radius&&e.days<=maxAge);arr.sort(sortMode==='closest'?(a,b)=>a.distance-b.distance||b.date-a.date:(a,b)=>b.date-a.date||a.distance-b.distance);const shown=arr.slice(0,limit);ui.summary.textContent=arr.length?`${arr.length.toLocaleString()} new · showing ${shown.length}`:'No new matches';if(!shown.length){ui.results.innerHTML='<div class="empty">No newly inspected establishments match these filters. Try a larger distance or older inspection window.</div>';return;}ui.results.innerHTML=shown.map((e,i)=>{const ds=e.distance<1?Math.round(e.distance*1000)+' m':e.distance.toFixed(1)+' km',map='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(e.name+' '+e.address+', Toronto'),cls=statusClass(e.result),rawType=e.type==='Dining-focused (type unavailable)'?'Dining-focused':e.type,type=esc(rawType),showType=rawType&&rawType!=='Dining-focused'&&!/\(derived\)$/i.test(rawType),typeHtml=showType?`<div class="establishment-type" title="${type}">${type}</div>`:'';return `<article class="item"><div class="rank">${i+1}</div><div class="topline"><div class="maincopy">${typeHtml}<h3>${esc(e.name)}</h3><div class="address"><a href="${map}" target="_blank" rel="noopener" aria-label="Open ${esc(e.address)} in Google Maps (opens in a new tab)"><span class="address-text">${esc(e.address)}</span><span class="external-mark" aria-hidden="true">↗</span></a></div></div></div><div class="distance">${ds}<small>away</small></div><div class="inspection"><div class="datebox"><strong>${fmtDate(e.date)}</strong><span>${ageLabel(e.days)}</span></div><div class="inspection-copy"><div class="statusline ${cls}">${esc(e.result||'Inspection recorded')}</div><div class="type">${esc(inspDetail(e))}</div></div></div></article>`;}).join('');}
