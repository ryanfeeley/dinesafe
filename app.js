...
function render(){updateFilterCount();if(!coords||!establishments)return;const radius=+ui.radius.value,maxInspections=+ui.maxInspections.value,limit=100;let arr=establishments.map(e=>({...e,distance:hav(coords.lat,coords.lon,e.lat,e.lon),days:ageDays(e.date)})).filter(e=>e.distance<=radius&&e.inspectionCount<=maxInspections);arr.sort(sortMode==='closest'?(a,b)=>a.distance-b.distance||b.date-a.date:(a,b)=>b.date-a.date||a.distance-b.distance);const shown=arr.slice(0,limit);ui.summary.textContent=arr.length?`${arr.length.toLocaleString()} matches · showing ${shown.length}`:'No matches';if(!shown.length){ui.results.innerHTML='<div class="empty">No establishments match these filters. Try a larger distance or higher inspection setting.</div>';return;}ui.results.innerHTML=shown.map(e=>{const ds=e.distance<1?Math.round(e.distance*1000)+' m':e.distance.toFixed(1)+' km',map='https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(e.name+' '+e.address+', Toronto'),cls=statusClass(e.result);return `<article class="item"><div class="topline"><div class="maincopy"><h3>${esc(e.name)}</h3><div class="address"><a href="${map}" target="_blank" rel="noopener" aria-label="Open ${esc(e.address)} in Google Maps (opens in a new tab)"><span class="address-text">${esc(e.address)}</span><span class="external-mark" aria-hidden="true">↗</span></a></div></div></div><div class="distance">${ds}<small>away</small></div><div class="datebox"><strong>${fmtDate(e.date)}</strong><span>${ageLabel(e.days)}</span></div><div class="inspection-copy"><div class="statusline ${cls}">${esc(e.result||'Inspection recorded')}</div><div class="type">${esc(inspectionSummary(e))}</div></div></article>`;}).join('');}
